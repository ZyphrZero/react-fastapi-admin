...
func ensureRoles(ctx context.Context, db *gorm.DB) error {
	var count int64
	if err := db.WithContext(ctx).Model(&database.Role{}).Count(&count).Error; err != nil {
		return err
	}
	if count > 0 {
		return nil
	...
	userDesc := "普通用户角色"
	roles := []database.Role{
		{Name: "管理员", Desc: &adminDesc, MenuPaths: database.JSONStringSlice(base.DefaultRoleMenuPaths("管理员")), APIIDs: database.JSONInt64Slice{}},
		{Name: "普通用户", Desc: &userDesc, MenuPaths: database.JSONStringSlice(base.DefaultRoleMenuPaths("普通用户")), APIIDs: database.JSONInt64Slice{}},
	}
...
func ensureAdmin(ctx context.Context, cfg *config.Config, db *gorm.DB) (*database.User, string, error) {
	var count int64
	if err := db.WithContext(ctx).Model(&database.User{}).Count(&count).Error; err != nil {
		return nil, "", err
	}
	if count > 0 {
		var existing database.User
		if err := db.WithContext(ctx).Where("username = ? AND is_superuser = ?", cfg.InitialAdminUsername, true).First(&existing).Error; err != nil {
			return nil, "", err
		}
		return &existing, "", nil
	}
